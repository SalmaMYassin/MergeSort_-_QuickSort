...
long double mergeTotal;             // to Calculates the duration of the mergeSort function
long double quickTotal;             // to Calculates the duration of the quickSort function
...
int main(){
	...
	//srand(time(NULL));				//makes sure that numbers are not the same in each run
	...
	for (int i = 0; i < size; i++){     // A loop to let the user enter Elements to be sorted
		...
		cout << "Enter number: ";
		cin >> Arr[i];
		//Arr[i] = rand() % 100;		// generates random numbers;
	}
	...
	display(Arr, size, 1);
	display(Arr2, size, 2);
	cout << "Time Elapsed for MergeSort: " << fixed << setprecision(8) << mergeTotal << "s" << endl;
	cout << "Time Elapsed for QuickSort: " << fixed << setprecision(8) << quickTotal << "s" << endl;
